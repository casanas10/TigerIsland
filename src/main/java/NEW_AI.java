...
public class NEW_AI {
    ...
    private BuildResult makeMove() {
        ...
        buildResult = buildATotoroSantuary();
    ...
    public BuildResult buildATotoroSantuary() {

        if (findAISettlements5orGreater(aiPlayer) != -1 ) {
    ...
    public MoveData BuildAction(NukeResult nukeResult, BuildResult buildResult) {
        ...
        if (buildResult.hexID == -1 || buildResult.hexID == 0){
            ...
            for (int i = 0; i < allPossibleTiles.size(); i++){
                ...
                } else if (builder.build(aiPlayer, islandMap, buildOption, pairs[2])){
